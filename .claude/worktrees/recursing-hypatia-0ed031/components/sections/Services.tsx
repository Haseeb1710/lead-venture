...
import { Lightbulb, Search, Sparkles, type LucideIcon } from "lucide-react"
          ...
          {SERVICES.map((s) => {
            ...
            return (
              <TabsContent key={s.id} value={s.id}>
                ...
                >
                  <div
                    className="w-16 h-16 mb-6 rounded-2xl bg-violet-100 text-violet-700 flex items-center justify-center"
                    aria-hidden="true"
                  >
                  ...
                  <ul className="space-y-4">
                    {s.features.map((f) => (
                      <li key={f} className="flex items-center gap-3 text-gray-700">
                        <span
                          className="w-6 h-6 bg-violet-100 text-violet-700 rounded-full flex items-center justify-center text-xs font-bold flex-shrink-0"
                          aria-hidden="true"
                        >
                          ✓
                        </span>
